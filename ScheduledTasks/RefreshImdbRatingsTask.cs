...
public class RefreshImdbRatingsTask : IScheduledTask
{
    ...
    public async Task ExecuteAsync(IProgress<double> progress, CancellationToken cancellationToken)
    {
        ...
        IReadOnlyList<BaseItem> seasonAverageEpisodes = Array.Empty<BaseItem>();
        if (config.IncludeSeries && config.IncludeSeasonAverages)
        {
            seasonAverageEpisodes = GetSeasonAverageEpisodes();
        }
        ...
        for (int i = 0; i < seasonAverageEpisodes.Count; i++)
        {
            var imdbId = seasonAverageEpisodes[i].GetProviderId(MediaBrowser.Model.Entities.MetadataProvider.Imdb);
            if (!string.IsNullOrWhiteSpace(imdbId))
            {
                libraryImdbIds.Add(imdbId);
            }
        ...
        _logger.LogInformation(
            "Found {ItemCount} library items with IMDb IDs ({DistinctIdCount} distinct IDs; {SeasonAverageEpisodeCount} episodes queried only for season averages)",
            items.Count,
            libraryImdbIds.Count,
            seasonAverageEpisodes.Count);
        ...
        if (config.IncludeSeries && config.IncludeSeasonAverages)
        {
            // Episodes are queried only for season-average calculation; they are not updated themselves.
            var episodeData = seasonAverageEpisodes
    ...
    private IReadOnlyList<BaseItem> GetSeasonAverageEpisodes()
    {
        var query = new InternalItemsQuery
        {
            HasImdbId = true,
